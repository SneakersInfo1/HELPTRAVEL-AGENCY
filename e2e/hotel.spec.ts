...
test.describe("B — mapa na desktopie", () => {
  ...
  test("widok LISTY nie pobiera biblioteki mapy (brief §29)", async ({ page }) => {
    ...
    await dismissCookies(page);
    await waitForScan(page);
    // Sama lista: ani kilobajta silnika mapy, ani jednego kafelka.
    expect(pobrane, `lista pobrała zasoby mapy: ${pobrane.join(", ")}`).toHaveLength(0);

    // Dopiero przełączenie na mapę ściąga bibliotekę i kafelki.
    await page.getByRole("button", { name: "Mapa", exact: true }).click();
    await expect(page.locator("canvas.maplibregl-canvas")).toBeVisible({ timeout: 20_000 });
    await page.waitForTimeout(2000);
    expect(pobrane.length).toBeGreaterThan(0);
  });
...
test.describe("D — pokoje", () => {
  ...
  test("klik w pokój otwiera modal ze zdjęciami i wariantami", async ({ page }) => {
    ...
    const dialog = page.getByRole("dialog").filter({ hasNot: page.getByText("PLIKI COOKIE") });
    await expect(dialog).toBeVisible();
    await expect(dialog.getByRole("heading", { name: /Szczegóły pokoju/ })).toBeVisible();
